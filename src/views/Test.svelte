    ...
    {#if alertOpen}
      <div v-if="alertOpen" class="text-white px-6 py-4 border-0 rounded relative mb-4 bg-orange-500">
        ...
        <span class="inline-block align-middle mr-8">
          <b class="capitalize">Step 1:</b> Align your sight with the center of grid. Then, draw blurred region with the pentool.
        </span>
        <button class="absolute bg-transparent text-2xl font-semibold leading-none right-0 top-0 mt-4 mr-4 outline-none focus:outline-none" on:click={closeAlert}>
          <span>×</span>
        </button>
      </div>
    {/if}

    <div class="flex flex-wrap justify-center bg-white shadow-xl rounded-lg  py-16 px-12 relative z-10">
      <div class="w-screen object-center">
        ...
        <div class="sm:block flex flex-col mt-10">
          <button on:click={clear} class="github-star sm:ml-1 text-white font-bold px-6 py-4 rounded outline-none focus:outline-none mr-1 mb-1 bg-gray-800 active:bg-gray-700 uppercase text-sm shadow hover:shadow-lg ease-linear transition-all duration-150">
            <i class="fas fa-trash text-lg mr-1"></i>
            <span>Clear</span>
          </button>

          
          <button on:click={get_image_data} class="github-star sm:ml-1 text-white font-bold px-6 py-4 rounded outline-none focus:outline-none mr-1 mb-1 bg-gray-800 active:bg-gray-700 uppercase text-sm shadow hover:shadow-lg ease-linear transition-all duration-150">
            <i class="fas fa-download text-lg mr-1"></i>
            <span>Download</span>
          </button>
